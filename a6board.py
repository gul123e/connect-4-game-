...
Our version of the game allows variati"""
The gameboard for the Connect-N game.
...
class Board:
    ...
    def __init__(self,rows=6,cols=7,streak=4):
        ...
        self._height=rows
        self._streak=streak
        self._board=[[NOBODY for i in range(self._width)] for i in range(self._height)]
        self._moves=[]
    ...
    def isFullColumn(self,col):
        ...
        """
        assert isinstance(col,int) and 0<=col<self._width
        return self.findAvailableRow(col)==self._height
...
#### TASK 4 ####
    ...
    def findAcross(self,r,c,leng):
        ...
        while c1 >= 0 and self._board[r][c1] == color:
            c1-=1      # We are going downwards# Back up one.
        c1= c1+1
        c2=c
    ...
    def findSWNE(self,r,c,leng):
        ...
        color = self._board[r][c]
        c1,r1=c,r
        while r1<self._height and c1>=0 and self._board[r1][c1]==color:
            r1+=1
            c1-=1
        c1+=1
        r1-=1
        r2,c2=r,c
        while r2>=0  and c2<self._width and self._board[r2][c2]==color:
            r2-=1
            c2+=1
        c2-=1
        r2+=1
        if dist(self,r1,c1,r2,c2)+1 >= leng:
            return (r1,c1,r2,c2)
        else:
            return None  
    ...
    def findNWSE(self,r,c,leng):
        ...
        color = self._board[r][c] #xcheck color
        r1,c1=r,c
        while r1>0 and c1>0 and self._board[r1][c1]==color:
            r1-=1
            c1-=1
        r1+=1 #move o ne stp  to match the chexk the last move 
        c1+=1
        r2,c2 =r,c
        while r2<self._height and c2<self._width and self._board[r2][c2]==color:
            r2+=1
            c2+=1
        r2-=1
        c2-=1
    ...
    def findWins(self,r,c):
        ...
        result=self.findVertical(r,c,streak)
        ...
        if result is not  None:
            return result
        result=self.findNWSE(r,c,streak)
        if result is not  None:
            return result
        return None
...
ons on some or all of these parameters.
...
class Board:
    ...
    def place(self,column,color):
        ...
        assert introcs.is_tkcolor(color)
        straight_line=self.findAvailableRow(column)
        if straight_line==self._height:
            return -1
        self._board[straight_line][column]=color
        self._movement.append((straight_line,column))
    ...
    def undoPlace(self):
        ...
        if not self._movement:
            return
        straight_line,col=self._movement.pop()
        self._board[straight_line][col]=""
