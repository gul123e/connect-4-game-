...
class Board:
    ...
    def __init__(self,rows=6,cols=7,streak=4):
        ...
        assert isinstance (streak,int) and 0<streak<=min(rows,cols)
...
#### TASK 4 ####
    ...
    def findSWNE(self,r,c,leng):
        ...
        color = self._board[r][c]
        row1,col1=r,c
        while (row1+1<self._height and col1-1 >= 0 and self._board[row1+1][col1-1]==color):
            row1+=1
            col1-=1
        
        row2,col2=r,c
        while (row2-1>=0  and col2+1 < self._width and self._board[row2-1][col2+1]==color):
            row2-=1
            col2+=1
        count=max(abs(row1-row2),abs(col1-col2))+1
    ...
    def findNWSE(self,r,c,leng):
        ...
        r1,c1=r,c
        while r1<self._height -1 and c1>0 and self._board[r1+1][c1-1]==color:
            r1+=1
            c1-=1
        #r1+=1
        # c1+=1
        r2,c2 =r,c
        while r2>0  and c2+1<self._width -1  and self._board[r2-1][c2+1]==color:
            r2-=1
            c2+=1
        if abs(r2-r1)+1 >= leng:
            return (r1,c1,r2,c2)
